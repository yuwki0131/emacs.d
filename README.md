...
|編集|検索|C-M-s|vr/isearch-forward|vr/isearch側の正規表現置換|
|編集|検索|C-M-r|vr/isearch-backward|vr/isearch側の正規表現置換|
...
|移動|バッファ内|M-p|scroll-up-in-place|スクロール(カーソル位置固定)|
|移動|バッファ内|M-n|scroll-down-in-place|スクロール(カーソル位置固定)|
|移動|バッファ内|M-g|goto-line|goto-line(１回)|
|移動|バッファ内|M-a|ace-jump-mode|ace jump mode|
|移動|バッファ内|C-.|goto-next-TODO|TODOへ移動|
|移動|バッファ内|C-?|highlight-symbol-next|シンボル移動　(highlight-symbol-mode)|
|移動|バッファ内|C-!|highlight-symbol-prev|シンボル移動　(highlight-symbol-mode)|
...
|編集|挿入|C-a C-e|extract-current-buffer-name|現在のファイル名を挿入|
